var plenigoSettings = plenigoSettings || {
    ...
    addValue: function (event) {
        ...
        if (strSettingValue.indexOf(setString) >= 0) {
            alert("Combination already exists.");
        } else {
            that.jqHidden[prefix].val(strSettingValue + "\n" + setString);
        }
    ...
    delValue: function (event) {
        ...
        var that = plenigoSettings;
        var prefix = event.data;
        var setString = that.jqEditor[prefix].find(":selected").val();
        var strSettingValue = that.jqHidden[prefix].val(); // DB value
        if (strSettingValue.indexOf(setString) >= 0) {
            strSettingValue = strSettingValue.replace(setString, "");
            strSettingValue = strSettingValue.replace("\n\n", "\n");
            that.jqHidden[prefix].val(strSettingValue);
        }
